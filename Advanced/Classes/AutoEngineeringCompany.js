...
// Constraints
// The input will never include more than 50 lines
// The car model will never be repeated
...
function autoEngineeringCompany(input = []) {
    ...
    return Object.entries(cars).map(x => `${x[0]}\n${Object.entries(x[1]).map(y => `###${y[0]} -> ${y[1]}`).join("\n")}`).join("\n");
}
